...
const EncoderNoPadding = std.base64.url_safe_no_pad.Encoder;
const DecoderNoPadding = std.base64.url_safe_no_pad.Decoder;
...
pub fn encodeNoPadding(alloc: std.mem.Allocator, src: []const u8) ![]const u8 {
    const encoded_length = Encoder.calcSize(src.len);
    const encoded_buffer = try alloc.alloc(u8, encoded_length);
    const encoded = EncoderNoPadding.encode(encoded_buffer, src);
    return encoded;
}

pub fn decodeNoPadding(alloc: std.mem.Allocator, encoded_buffer: []const u8) ![]u8 {
    const decoded_length = try Decoder.calcSizeForSlice(encoded_buffer);
    const decoded_buffer = try alloc.alloc(u8, decoded_length);
    try DecoderNoPadding.decode(decoded_buffer, encoded_buffer);
    return decoded_buffer;
}
